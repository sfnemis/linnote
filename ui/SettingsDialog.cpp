...
void SettingsDialog::updateDynamicStyles() {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Cross-Platform Widget Styling
  // Ubuntu GNOME doesn't inherit Qt dark theme properly, so we set explicit
  // colors for all input widgets to ensure consistent appearance.
  // ═══════════════════════════════════════════════════════════════════════════

  QString bgColor = isDark ? "#1e1e2e" : "#f5f5f5";
  QString textColor = isDark ? "#cdd6f4" : "#2e3436";
  QString inputBg = isDark ? "#313244" : "#ffffff";
  QString inputBorder = isDark ? "#45475a" : "#dcdcdc";
  ...
  m_sidebar->setStyleSheet(
      QString("QListWidget { border: none; padding: 8px; "
              ...
              "margin: 1px 4px; border: 1px solid transparent; color: %2; }"
              "QListWidget::item:hover { background: %3; "
              "border: 1px solid %4; color: %2; }"
              "QListWidget::item:selected { background-color: #3584e4; "
              "color: #ffffff; font-weight: bold; "
              "border: 1px solid #3584e4; }")
          .arg(bgColor, textColor, hoverBg, hoverBorder, borderColor));

  // Main dialog and pages background
  QString dialogStyle =
      QString(
          "QDialog, QStackedWidget, QWidget, QScrollArea { "
          ...
          // QComboBox
          "QComboBox { background-color: %3; color: %2; border: 1px solid %4; "
          "border-radius: 4px; padding: 6px 8px; min-width: 100px; }"
          "QComboBox::drop-down { border: none; width: 20px; }"
          "QComboBox::down-arrow { width: 12px; height: 12px; }"
          "QComboBox QAbstractItemView { background-color: %3; color: %2; "
          "border: 1px solid %4; selection-background-color: %7; }"
          // QPushButton
          "QPushButton { background-color: %3; color: %2; border: 1px solid "
          "%4; "
          "border-radius: 4px; padding: 8px 16px; }"
          "QPushButton:hover { background-color: %5; border: 1px solid %6; }"
          "QPushButton:pressed { background-color: %4; }"
          // QCheckBox
          "QCheckBox { color: %2; spacing: 8px; }"
          "QCheckBox::indicator { width: 18px; height: 18px; border-radius: "
          "4px; "
          "border: 1px solid %4; background-color: %3; }"
          "QCheckBox::indicator:checked { background-color: %7; border: 1px "
          "solid %7; }"
          // QSpinBox
          "QSpinBox { background-color: %3; color: %2; border: 1px solid %4; "
          "border-radius: 4px; padding: 4px 8px; }"
          // QSlider
          "QSlider::groove:horizontal { height: 6px; background: %4; "
          "border-radius: 3px; }"
          "QSlider::handle:horizontal { width: 16px; height: 16px; margin: "
          "-5px 0; "
          "border-radius: 8px; background: %7; }"
          "QSlider::sub-page:horizontal { background: %7; border-radius: 3px; }"
          // QKeySequenceEdit
          "QKeySequenceEdit { background-color: %3; color: %2; border: 1px "
          "solid %4; "
          "border-radius: 4px; padding: 4px 8px; }"
          // QLabel (just color)
          ...
          "QScrollBar::handle:vertical { background: %4; border-radius: 5px; "
          "min-height: 20px; }"
          "QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { "
          "height: 0; }")
          .arg(bgColor, textColor, inputBg, inputBorder, hoverBg, hoverBorder,
               accentColor);
